...
const keyPressedHandler = (_, key) => {
    if (key && selectOption.inSelectType) {
        ...
        if (key.name === 'down' && selectOption.selectIndex < optionLength) {
            ...
            selectOption.callback(selectOption.selects, selectOption.modelName)
            selectOption.close()
        }
    }
}
...
const ansiColors = (text, color) => {
    ...
    if (colors[color]) {
        `\x1b[${colors[color]}m${text}\x1b[0m`
    }
    // default for colors not included
    return `\x1b[32m${text}\x1b[0m`
}

selectOption.init = (callback, modelName) => {
    selectOption.callback = callback
    selectOption.modelName = modelName
...
selectOption.createOptionMenu = () => {
    ...
    for (let i = 0; i < optionLength; i++) {
        const selectedOption = i === selectOption.selectIndex            ? `${cursorColor} ${selectOption.options[i]}`            : selectOption.options[i]
